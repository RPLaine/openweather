...
function App() {
  ...
  */
  var PrecipitionValue = (data) => {
    var value = 0;
    if ("snow" in data){
      value += parseInt(Object.values(data.snow)[0]);
    }
    else{
      if ("rain" in data){
        value += parseInt(Object.values(data.rain)[0]);
      }
    }
  ...
  /*
  Returns "d" of "n" depending if there is sunlight.
  Affects icons: Colored versions for daylight hours; grayscale for night.
  Could be improved if timed according to sunset and sunrise.
  */
    ...
    return(
      <div>
        <div className="increment">
          ...
          <div><img src={"http://openweathermap.org/img/wn/" + data.weather[0].icon.substring(0,2) + DayOrNight() + ".png"} alt="icon" /></div>
